...
function sumOfCodes(str) {
  if (typeof str === 'string') {
    ...
    for (let i = 0; i < str.length; i += 1) {
      const numChar = str[i].charCodeAt();
      sumChar += numChar;
    }
    return sumChar;
  }
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let j = 0; j < vowels.length; j += 1) {
      const index = vowels[j];
      if (str[i].includes(index)) count += 1;
    }
  }
  return count;
}
...
function isPalindrome(str) {
  const newStr = str.toLowerCase().replaceAll(/[.,!?\s]/g, '');
...
function findLongestWord(sentence) {
  ...
  const arrOfLengthStr = [];
  let indexLongestLength;
  for (let i = 0; i < arrStr.length; i += 1) {
    arrOfLengthStr.push(arrStr[i].length);
  }
  for (let i = 0; i < arrOfLengthStr.length - 1; i += 1) {
    for (let j = i + 1; j < arrOfLengthStr.length; j += 1) {
      if (arrOfLengthStr[i] > arrOfLengthStr[j]) {
        indexLongestLength = arrOfLengthStr[i];
      } else indexLongestLength = arrOfLengthStr[j];
    }
  }

  const indexLongestWorld = arrOfLengthStr.indexOf(indexLongestLength);
  return arrStr[indexLongestWorld];
}
...
function reverseWords(str) {
  ...
  const newArr = [];
  for (let i = 0; i < arrStr.length; i += 1) {
    const reverseWorld = arrStr[i].split('').reverse().join('');
    newArr.push(reverseWorld);
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const indexLetter = alphabet.indexOf(str[i]);

    if (indexLetter >= 0) {
      newStr += rot13[indexLetter];
    } else {
      newStr += str[i];
    }
  }
